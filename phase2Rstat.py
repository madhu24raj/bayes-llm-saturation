...
# ==========================================
# 3. Random Walk Metropolis Sampler (Multi-Chain)
# ==========================================
def metropolis_sampler_hierarchical(grouped_data, start_theta, iters=25000, burn_in=5000, chain_id=1):
    print(f"Starting Phase 2 Chain {chain_id} ({iters} iterations)...")
...
def run_multiple_chains_hierarchical(grouped_data, n_chains=4, iters=30000, burn_in=10000):
    ...
    for c in range(n_chains):
        ...
        start_theta[5:] = np.random.normal(0, 5, J)                 # b0_j

        chain = metropolis_sampler_hierarchical(
            grouped_data, start_theta, iters=iters, burn_in=burn_in, chain_id=c+1
        )
        chains.append(chain)
...
# Run and combine
multi_chain_trace = run_multiple_chains_hierarchical(grouped_data, n_chains=4, iters=30000, burn_in=10000)
...
# ==========================================
global_param_names = ['beta0', 'beta1', 'rho', 'sigma', 'tau0']
all_param_names = global_param_names + [f'b0_{group_names[j]}' for j in range(J)]

print("\n=== Phase 2 Convergence Diagnostics (Gelman-Rubin) ===")
for i, name in enumerate(all_param_names):
    status = "✅ Converged" if r_hat_stats[i] < 1.05 else "❌ Needs attention"
    print(f"{name:20} R-hat: {r_hat_stats[i]:.4f}  {status}")
